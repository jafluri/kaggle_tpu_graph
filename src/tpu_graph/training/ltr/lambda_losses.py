...
from .utils import batch_pairs as _batch_pairs
from .utils import rank_by_score as _rank_by_score
...
class LambdaLoss(_torch.nn.Module):
    ...
    def _loss_per_doc_pair(
        self, score_pairs: _torch.FloatTensor, rel_pairs: _torch.LongTensor, n: _torch.LongTensor
    ) -> _torch.FloatTensor:
    ...
    def _loss_reduction(self, loss_pairs: _torch.FloatTensor) -> _torch.FloatTensor:
    ...
    def forward(
        self, scores: _torch.FloatTensor, relevance: _torch.LongTensor, n: _torch.LongTensor
    ) -> _torch.FloatTensor:
...
class LambdaNDCGLoss2(LambdaLoss):
    ...
    def _loss_per_doc_pair(self, score_pairs, rel_pairs, n):
        ...
        arange = _torch.arange(score_pairs.shape[1] + 1, device=score_pairs.device)
        discounts = _torch.log2(2.0 + arange)
...
def _ndcg_gains(
    score_pairs: _torch.FloatTensor, rel_pairs: _torch.LongTensor, n: _torch.LongTensor, exp: bool = True
) -> _torch.FloatTensor:
    gains = rel_pairs[:, :, :, :]
    if exp:
        gains = (2**gains) - 1.0
...
def _max_dcg(relevance: _torch.FloatTensor, n: _torch.LongTensor, exp: bool = True) -> _torch.FloatTensor:
    ranking = _rank_by_score(relevance.double(), n)
    arange = _torch.arange(ranking.shape[1], device=relevance.device)
    discounts = _torch.log2(2.0 + arange)
