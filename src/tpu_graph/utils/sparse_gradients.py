...
def _sort_coo_indices(indices):
    """
    Sorts COO (Coordinate List Format) indices in ascending order and returns a permutation tensor that indicates the
     indices in the original data that result in a sorted tensor.

    This function can support both unbatched and batched COO indices, and essentially performs the same operation as
    .coalesce() called on a COO tensor.
    The advantage is that COO coordinates can be sorted prior to conversion to CSR, without having to use the
     torch.sparse_coo_tensor object which only supports int64 indices.
...
def convert_coo_to_csr_indices_values(coo_indices, num_rows, values=None):
    ...
    This function sorts the row and column indices (similar to torch.sparse_coo_tensor.coalesce())
    and then compresses the row indices to CSR crow indices.
    ...
    if coo_indices.shape[0] < 2:
        ...
        raise ValueError(
            f"Current implementation only supports single batch diomension, "
            f"therefore indices tensor must have at most 3 rows (batch, row and column indices). "
            f"Got {coo_indices.shape[0]} rows."
        )
...
def sparse_eye(
    ...
    *,
    ...
    Args:
        size (tuple): Tuple specifying the dimensions of the sparse matrix. The size can be either (num_rows, num_cols)
        for an unbatched matrix or (batch_size, num_rows, num_cols) for a batched matrix. The number of rows and columns
         must be equal.
        ...
        layout (:class:`torch.layout`, optional): The desired layout of returned SparseTensor.
         Default is torch.sparse_coo_tensor.
